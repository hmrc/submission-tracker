...
class WSHttpImpl @Inject()(@Named("appName") val appName: String, val auditConnector: AuditConnector, val actorSystem: ActorSystem, config: Configuration) extends HttpClient with WSGet
  with WSPut
  with WSPost
  with WSDelete
  with WSPatch
