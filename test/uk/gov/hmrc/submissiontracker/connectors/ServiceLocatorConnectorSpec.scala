...
class ServiceLocatorConnectorSpec extends WordSpecLike with Matchers with MockFactory with FutureAwaits with DefaultAwaitTimeout {
  ...
  "register" should {
    "register the JSON API Definition into the Service Locator" in new Setup {
      ...
      val registration = Registration(
        serviceName = "api-microservice-template",
        serviceUrl  = "http://api-microservice-template.service",
        metadata    = Some(Map("third-party-api" -> "true")))

      (mockHttp
        .POST(_: String, _: Registration, _: Seq[(String, String)])(
          _: Writes[Registration],
          _: HttpReads[HttpResponse],
          _: HeaderCarrier,
          _: ExecutionContext))
        .expects(s"${connector.serviceUrl}/registration", registration, *, *, *, *, *)
    ...
    "fail registering in service locator" in new Setup {
      ...
      (mockHttp
        .POST(_: String, _: Registration, _: Seq[(String, String)])(
          ...
          _: ExecutionContext))
        .expects(s"${connector.serviceUrl}/registration", registration, *, *, *, *, *)
        .returning(Future.failed(serviceLocatorException))
